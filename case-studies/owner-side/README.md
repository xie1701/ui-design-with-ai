...
这是一次真实设计流程的公开案例记录，用于展示 Skill 如何发散、收敛、复核和记录风险。为避免重新识别，案例中的人物姓名、家庭构成、儿童作息、户型面积、预算和其他生活细节均为**合成演示数据**，不对应任何真实家庭；产品名称也已泛化。它不是受控实验，不能单独证明跨项目或新手使用的稳定性。
...
- 数据边界：全部人物与数值均为合成演示内容，不是去标识后保留的真实个人数据
- 工程检查：历史上做过 DOM/脚本/多视口检查，但未保存完整可复核输出；缺证据项目已在 QA 中降为 `NOT TESTED`
